...
} from '../../../packages/widget-contract/dist/public-api.js';
...
export async function checkBuiltWidget(outputDirectory, browserPath) {
  ...
  try {
    ...
    for (const tag of [build.manifest.elementTag, build.manifest.settingsElementTag]) {
...
async function readManifest(manifestPath) {
  ...
  try {
    ...
    throw new Error(
      `Production output is missing ${MANIFEST_FILE_NAME}. Run the Weather Widget build before checking it.`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Production ${MANIFEST_FILE_NAME} is not valid JSON.`,
    );
  }
}
...
function runProcess(command, arguments_) {
  return new Promise((resolveProcess, rejectProcess) => {
    const browser = spawn(command, arguments_, { stdio: ['ignore', 'pipe', 'pipe'] });
    let output = '';
    let errors = '';
    ...
    browser.once('close', (code) => {
      ...
      code === 0
        ...
        : rejectProcess(new Error(`Headless browser exited with ${code}: ${errors}`));
    });
  });
}
...
async function main() {
  ...
  console.log(
    'Weather Widget output is static-hosting-ready and conforms to the browser Widget contract.',
  );
}
